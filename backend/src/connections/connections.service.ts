...
export class ConnectionsService {
    ...
    private async fetchRichTextsForScores(scores: any): Promise<Record<string, string>> {
        ...
        await Promise.all(mapTraits.map(async (t) => {
            ...
            // Prioridade 2: Interpretação Padrão
            const fallback = await this.prisma.bigFiveInterpretativeText.findFirst({
                where: { traitKey: t.label, scoreRange: level, category: { in: ['TEXT_INTERPRETATION', 'EXPERT_SYNTHESIS'] } },
                orderBy: { createdAt: 'desc' }
            });

            if (fallback?.text) richMap[t.label] = fallback.text;
        }));
