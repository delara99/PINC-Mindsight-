...
export default function CandidatesPage() {
    ...
    const fetchEmployees = async () => {
        try {
            ...
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/business/employees`, {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            const token = localStorage.getItem('accessToken');
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/business/employees`, {
                name: newName,
                email: newEmail
            }, {
                headers: { Authorization: `Bearer ${token}` }
