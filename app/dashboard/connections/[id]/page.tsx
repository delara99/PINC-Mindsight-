    ...
    const sendCoachMessageMutation = useMutation({
        mutationFn: async (message: string) => {
            ...
            const res = await fetch(`${API_URL}/api/v1/ai/chat`, {
                ...
                context: `CONNECTION:${id}` // Context isolation
            })
        });
...
// Carregar histórico de chat específico desta conexão ao abrir
useEffect(() => {
    if (!aiSummary) return; // Só carrega se o coach estiver ativo

    const fetchHistory = async () => {
        try {
            const res = await fetch(`${API_URL}/api/v1/ai/history?context=CONNECTION:${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.ok) {
                const data = await res.json();
                if (Array.isArray(data) && data.length > 0) {
                    setCoachHistory(data.map((msg: any) => ({
                        role: msg.role,
                        content: msg.content
                    })));
                }
